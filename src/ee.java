...
final class ee {
    ...
    static final boolean switchWorld(int worldId, String string, int i) {
        e++;
        if (((Signlink) nda.signlink).r) {
            gm.world = new World();
            ((World) gm.world).worldID = worldId;
            ((World) gm.world).lobbyAddress = string;
            if (hs.modeWhere != oba.pk) {
                ((World) gm.world).mainPort = ((World) gm.world).worldID + 40000;
                ((World) gm.world).worldPort = ((World) gm.world).worldID + 50000;
            }
            if (worldId < il.o.length && il.o[worldId] != null)
                cd.worldFlags = ((he) il.o[worldId]).e;
            return true;
        }
        String string_1_ = "";
        if (oba.pk != hs.modeWhere)
            string_1_ = ":" + (worldId + 7000);
        String string_2_ = "";
        if (i != 55)
            switchWorld((int) 81, null, 111);
        if (ow.settings != null)
