...
final class kea extends Canvas {
     ...
     * @param versionWM
     */
    public static void setWM(boolean bool, byte b, int i, int j, int versionWM) {
        cga.v = 0L;
        c++;
        int k = ia.getWMVersion(b ^ 0x13);
        if (versionWM == 3 || k == 3) {
            bool = true;
        }
        if (!lg.r.B()) {
        ...
        oba.a(k, j, bool, -101, i, versionWM);
        if (b != 126) {
            kea.i = null;
        }
    }
