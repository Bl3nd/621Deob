...
final class cc implements fa {
    ...
    public static void saveVarcs(boolean bool) {
        saveVarcs++;
        FileOnDisk fileOnDisk = null;
        try {
            fileOnDisk = Signlink.createDatFile("2", (byte) -101);
            BytesParser bytesParser = new BytesParser(6 * iv.t + 3);
            bytesParser.writeByte(1, (byte) -127);
            System.out.println("[cc] method height");
            bytesParser.writeShort(iv.t, 13469);
            for (int index = 0; index < rr.h.length; index++) {
                if (jm.i[index]) {
                    bytesParser.writeShort(index, 13469);
                    bytesParser.writeInt(rr.h[index], -94);
                }
            }
        ...
        hs.currentTime = us.getCurrentTime(119);
        dv.d = bool;
    }
            ...
            if (up.thisPlayer == mouseOverPlayer) {
                if (ba.c && (0x10 & Class_jc.db) != 0) {
                    lea.setClickMenuOption(0, false, 0, -1, true, uk.d, i - 6466, 0L,
                            Class_ag.k + " -> <col=ffffff>" + GameText.selfText.getString(cba.languageID),
                            2, Class_kd.itemName);
                    nu.l++;
                }
            } else {
                ...
                if (((Player) mouseOverPlayer).ud != 0) {
                    if ((((Player) mouseOverPlayer).ud ^ 0xffffffff) != 0)
                        string = (mouseOverPlayer.getUserDisplayName((byte) -124, true) + " ("
                                + GameText.skillText.getString(cba.languageID)
                    ...
                    String string_13_
                            = (Class_pf.O != me.q ? GameText.levelText.getString(cba.languageID)
                            : GameText.ratingText.getString(cba.languageID));
                    if (((Player) mouseOverPlayer).playerCombatLevel > ((Player) mouseOverPlayer).combatLevel)
                        string
                                ...
                                + (((Player) mouseOverPlayer).playerCombatLevel - ((Player) mouseOverPlayer).combatLevel) + ")");
                    else
                        string
                ...
                if (ba.c && !bool && (0x8 & Class_jc.db) != 0) {
                    lea.setClickMenuOption(0, false, 0, -1, true, uk.d, 126,
                            ...
                            Class_ag.k + " -> <col=ffffff>" + string, 4, Class_kd.itemName);
                    gba.y++;
                }
                if (!bool) {
                    for (int i_14_ = 7; i_14_ >= 0; i_14_--) {
                        if (GameOutPacket.m[i_14_] != null) {
                            ...
                            if (me.q == wa.t
                                    ...
                                    (GameText.attackText.getString(cba.languageID)))) {
