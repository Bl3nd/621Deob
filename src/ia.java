...
final class ia extends of implements MouseListener, MouseMotionListener {
    ...
    private final void a(int i, byte i_2_, int i_3_, int i_4_, int i_5_) {
        ...
        ((mo) var_mo).D = i;
        ((mo) var_mo).v = i_4_;
        ((mo) var_mo).p = us.getCurrentTime(98);
        L.b(var_mo, (byte) 93);
    }
    ...
    public static int getWMVersion(int i) {
        x++;
        if (em.a != null) {
            return 3;
        }
    ...
    static final void sendConsoleCommandText(int i, String string) {
        if (Node.b == null)
            Interface.c(80);
        q++;
        ru.gmtCalendar.setTime(new Date(us.getCurrentTime(118)));
        if (i != 4)
            a(-100, -116);
